...
1. STRIP OUT ALL existing comments (French, English, TODOs, commented-out code).
2. ADD A STRICT, MODERATE LEVEL OF ENGLISH DOCUMENTATION:
   - For TypeScript/JavaScript/React: Add exactly ONE concise JSDoc-style comment block (/** ... */) ABOVE every exported function, component, interface, or major variable. 
   - STRICTLY PROHIBITED: Do NOT add any inline comments inside the body of functions, hooks, or JSX. Do not over-explain.
   - For CSS files: Add concise standard comments (/* ... */) above major sections or classes.
   - For JSON/HTML: Do not add comments if the format does not properly support it, just return the clean code.
3. DO NOT alter any functional code, imports, variable names, logic, or formatting.
4. Output ONLY the clean code. Do NOT wrap the code in markdown fences like \`\`\`tsx or \`\`\`css.

Example of expected JS/TS commenting style:
...
async function processFile(filePath, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // Ignore les fichiers très courts ou vides pour gagner du temps
...
// 📁 LISTE EXHAUSTIVE DE TOUS LES FICHIERS DU FRONTEND (D'après ton arborescence)
...
async function run() {
  console.log(`🚀 Starting automated English re-commenting process with ${MODEL} via native fetch...\n`);
  for (const file of filesToProcess) {
    const fullPath = path.resolve(file);
    if (fs.existsSync(fullPath)) {
      await processFile(fullPath);
      await sleep(2500); // Pause de 2.5s pour éviter le rate-limiting
    } else {
      console.warn(`⚠️ File not found (Check your path): ${file}`);
    }
  }
  console.log('\n🎉 Done! All targeted files have been cleaned and appropriately commented.');
}
